...
% cell/1
% cell(?Cell)
pos(A, B) :- member(A, [1, 2, 3]), member(B, [1, 2, 3]).
cell(pos(A, B)):- pos(A, B).
...
all_pairs(K) :- findall((P1, P2), (cell(P1), cell(P2), P1 \= P2), Tmp), remove_duplicates(Tmp, [], K).
exclude_predicate(Pair, (Pair1, Pair2)) :- Pair == Pair1; Pair == Pair2.
exclude_predicate(Color, P1, P2, C, (Pair1, Pair2)) :- (P1, P2, C) == (Pair1, Pair2, Color); (P1, P2, C) == (Pair1, Pair2, Color).
valid_pairs_h([], Pairs, Color, PairsOut) :- PairsOut = Pairs.
valid_pairs_h([H | T], Pairs, Color, PairsOut) :-
    ((H = classic(P, C), exclude(exclude_predicate(P), Pairs, Tmp));
    (H = quantum(P1, P2, C), exclude(exclude_predicate(Color, P1, P2, C), Pairs, Tmp))), !,
    valid_pairs_h(T, Tmp, Color, ASDF), PairsOut = ASDF.
remove_duplicates([], Collector, Out) :- Out = Collector. 
remove_duplicates([(P1, P2) | Others], Collector, Out) :- ((\+member((P2, P1), Collector)) -> remove_duplicates(Others, [(P1, P2) | Collector], Out); remove_duplicates(Others, Collector, Out)).
valid_pairs(State, Color, Pairs) :- all_pairs(K), valid_pairs_h(State, K, Color, Pairs).
...
% winner/2
% winner(+State, -Colors)
winnerFunction(State, Color):-
    (member(classic(pos(X, 1),Color),State), member(classic(pos(X,2),Color),State), member(classic(pos(X,3),Color),State), member(X, [1, 2, 3]));
    (member(classic(pos(1,Y),Color),State), member(classic(pos(2,Y),Color),State), member(classic(pos(3,Y),Color),State), member(Y, [1, 2, 3]));
    member(classic(pos(1, 1),Color),State), member(classic(pos(2, 2),Color),State), member(classic(pos(3, 3),Color),State);
    member(classic(pos(1,3),Color),State), member(classic(pos(2,2),Color),State), member(classic(pos(3,1),Color),State).
    
winner(_, []) :- fail.
winner(State, Winner) :- findall(X, winnerFunction(State, X), Tmp), Tmp \= [], Winner = Tmp.
...
is_edge(State, Curr, Next) :- member(quantum(Curr, Next, _), State); member(quantum(Next, Curr, _), State).

has_cycle(State, Curr, Visited) :-
  member(Curr, Visited),
  !. 
has_cycle(State, Curr, Visited) :-
  is_edge(State, Curr , Next),
  has_cycle(State, Next, [Curr|Visited]) .
...
creeazaQuantica(quantum(_, _, Color), Cell, Piesa) :- 
    Piesa = classic(Cell, Color).

collapse(State, Piece, Cell, NewState) :-
    creeazaQuantica(Piece, Cell, Classica),
    delete(State, Piece, StateFaraPiece),
    append(StateFaraPiece, [Classica], StateCuClassic),
    findall(K, (K = quantum(P1, P2, Color), member(K, StateCuClassic), (P1 == Cell; P2 == Cell)), Lista),
